...
function onCreate(req, resp) {
	if (!req.query.a_id) {
		writeResponse(resp, {
			Success: false,
			Err: "a_id not specified."
		});
		return;
	}

	account_collection.findOne({
		a_id: req.query.a_id
	}, function (err, data) {
		if (err) {
...
function onLogin(req, resp) {
	...
	account_collection.findOne({
		...
		if (data) {
			...
			writeResponse(resp, {
				Success: false,
				Err: "Account not found(cannot login)"
			});
		}
	});
}
		...
		if (from_account) {
			...
			if (curr_ip === from_account.user_ip) {
				if (from_account.isOnline === true) {
					...
					account_collection.findOne({
						a_id: req.query.to_id
					}, function (err, to_account) {
						if (err) {
							console.log("Error occur on query: " + err);
							writeResponse(resp, {
								Success: false,
...
function createAccount(info, resp) {
	...
	createAccountCmd.stdout.once('data', function (data) {
		...
		if (!data.result) {
			...
			writeResponse(resp, {
				Success: false,
				Err: "Geth Error When Creating Account"
			});
		} else {
	...
	account_collection.findOne({
		...
		if (data) {
			...
			if (data.isOnline === true && curr_ip === data.user_ip) {
				...
				writeResponse(resp, {
					Success: false,
					Err: "You cannot logout other's account!"
				});
			}
		} else {
...
function checkCurrentAccountBalance(addr, resp) {
	...
	checkBalanceCmd.stdout.once('data', function (data) {
		...
		if (!data.result) {
			...
			writeResponse(resp, {
				Success: false,
				Err: "Geth error on checking balance"
			});
		} else {
	...
	transferCmd.stdout.once('data', function (data) {
		...
		if (!data.result) {
			...
			if (data.error.code === -32000) {
				writeResponse(resp, {
					Success: false,
					Err: "Cannot transfer such large amount"
				});
			} else {
				writeResponse(resp, {
					Success: false,
					Err: "Geth error on transfer"
				});
			}
		} else {
